...
if [ $# -eq 3 ]; then
	...
	if [ $# -eq 2 ]; then
		...
		read -p 'Please input one of the above compute host name to boot a SLAVE on (optional): ' compute_host
		...
		echo ''
		# Read new VM name
		read -p 'Please input a SLAVE host name other than what is above (hadoop-slave1): ' VM_name
